...
from model.dpmsolver_illusion import model_wrapper_illusion, DPM_Solver_illusion
...
def extract_camera_vectors(c2w):
    """
    Extracts the camera's right, up, forward vectors and position from a 4x4 camera-to-world matrix.
    
    Returns:
    - right: 3-element vector (X-axis)
    - up: 3-element vector (Y-axis)
    - forward: 3-element vector (Z-axis, negative for forward direction)
    - position: 3-element vector (translation)
    """
...
def save_camera_infos_to_json(c2w_list, json_filename):
    """
    Processes a list of camera-to-world matrices and saves camera vectors + positions into a JSON file.

    Parameters:
    - c2w_list: list of 4x4 numpy arrays or nested lists representing c2w matrices.
    - json_filename: output JSON file path.
    """
    camera_infos = []

    # Iterate over all c2w matrices and extract vectors
    for idx, c2w in enumerate(c2w_list):
        right, up, forward, position = extract_camera_vectors(c2w)
        ...
        print(f"Camera {idx} vectors added to JSON.")

    # Write the list of camera infos to JSON file
    with open(json_filename, 'w') as json_file:
        json.dump(camera_infos, json_file, indent=4)

    print(f"\nSuccessfully saved {len(c2w_list)} camera infos to '{json_filename}'.")
...
def main():
    ...
    if args.text:
        ...
        text_features2 = clip_text_encoder.encode(args.text2)
    ...
    for _ in tqdm(range(num_batch_per_rank)):
        ...
        condition2, unconditional_condition2 = {}, {}
        if class_cond:
        ...
        if text_cond:
            ...
            condition2['cond_text'] = text_features2
            unconditional_condition2['cond_text'] = torch.zeros_like(text_features2)
        ...
        model_fn = model_wrapper_illusion(
            model,
            noise_schedule,
            model_type=MODEL_TYPES[model_and_diffusion_config['diffusion']['predict_type']],
            model_kwargs={},
        ...
        with torch.no_grad():
            ...
            for i, cam_info in enumerate(model_kwargs["cams"]):
                ...
                c2ws.append(cam_info["c2w"])
        
            # save_camera_infos_to_json(c2ws, "camera_infos.json")
...
def create_argparser():
    ...
    parser.add_argument("--text", type=str, default="A dog.")
    parser.add_argument("--text2", type=str, default="A cat.")
    parser.add_argument("--w_id", type=int, default=1)
    parser.add_argument("--w_rotated", type=int, default=1)
    # choice: 0, 90, 180, 270
    parser.add_argument("--rotation_angle", type=int, default=0, choices=[0, 90, 180, 270])
    return parser
